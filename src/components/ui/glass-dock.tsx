...
// Helper to check if MorphSVGPlugin is active
const getHasMorph = () => {
  return typeof window !== 'undefined' && gsap.plugins && gsap.plugins.morphSVG;
};
...
const MorphingIcon = ({ type, isActive, onClick, onMouseEnter }: { type: string, isActive: boolean, onClick: () => void, onMouseEnter?: () => void }) => {
    ...
    const animateHome = () => {
        ...
        gsap.killTweensOf([button, path, path.parentElement]);
        gsap.to(button, {
    ...
    const handleMouseLeave = () => {
        gsap.killTweensOf([buttonRef.current, pathRef.current, pathRef.current?.parentElement]);
        if (type === 'home') {
            gsap.to(buttonRef.current, { "--tab-bar-home-scale": 0, "--tab-bar-home-opacity": 0, duration: 0.3, ease: 'power2.out' });
        } else if (type === 'blog') {
            gsap.to(buttonRef.current, { "--tab-bar-blog-scale": 0, "--tab-bar-blog-opacity": 0, duration: 0.3, ease: 'power2.out' });
        } else if (type === 'marker') {
            gsap.to(buttonRef.current, { "--tab-bar-marker-scale": 0, "--tab-bar-marker-opacity": 0, duration: 0.3, ease: 'power2.out' });
        } else if (type === 'email') {
            gsap.to(buttonRef.current, { "--tab-bar-email-scale": 0, "--tab-bar-email-opacity": 0, duration: 0.3, ease: 'power2.out' });
        } else if (type === 'linkedin') {
            gsap.to(buttonRef.current, { "--tab-bar-linkedin-scale": 0, "--tab-bar-linkedin-opacity": 0, duration: 0.3, ease: 'power2.out' });
        } else if (type === 'x') {
            gsap.to(buttonRef.current, { "--tab-bar-x-scale": 0, "--tab-bar-x-opacity": 0, duration: 0.3, ease: 'power2.out' });
        } else if (type === 'github') {
            gsap.to(buttonRef.current, { "--tab-bar-github-scale": 0, "--tab-bar-github-opacity": 0, duration: 0.3, ease: 'power2.out' });
        }
        if (pathRef.current?.parentElement) {
            gsap.to(pathRef.current.parentElement, { scale: 1, rotation: 0, y: 0, duration: 0.3, ease: 'power2.out' });
        }
    };
    ...
    if (type === 'github') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className={cn("github", isActive ? "active" : "")}>
                <svg viewBox="0 0 24 24">
...
export const GlassDock = React.forwardRef<HTMLDivElement, GlassDockProps>(
    (
        ...
        const getTooltipPosition = (index: number) => index * 56 + 12;
